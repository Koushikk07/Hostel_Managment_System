...
app.post("/register", async (req, res) => {
  try {
    ...
    if (!/^[0-9]+$/.test(roll_no)) {
      return res.json({ success: false, message: "Roll number must contain only numbers" });
    }

    // Check if user already exists
    const [exists] = await db.execute("SELECT id FROM users WHERE roll_no = ?", [roll_no]);
    if (exists.length > 0) {
      return res.json({ success: false, message: "User with this roll number already exists" });
    }
...
app.post("/register/request-otp", async (req, res) => {
  try {
    ...
    const toNumber = phone.startsWith("+91") ? phone : "+91" + phone.replace(/\D/g, "");

    // Generate OTP
    const otp = Math.floor(100000 + Math.random() * 900000).toString();

    // Save OTP with 5 min expiration
    otpStore.set(email, { otp, data: req.body, expires: Date.now() + 5 * 60 * 1000 });
...
app.post("/register/verify-otp", async (req, res) => {
  try {
    ...
    if (!entry) return res.json({ success: false, message: "No OTP request found" });
    if (entry.expires < Date.now()) return res.json({ success: false, message: "OTP expired" });
    if (entry.otp !== otp) return res.json({ success: false, message: "Invalid OTP" });
...
app.get("/api/finance/:roll_no", checkStudent, async (req, res) => {
  ...
  try {
    ...
    const years = Object.keys(yearsMap).map((year) => {
      let totalFee = 0, totalPaid = 0, totalScholarship = 0;
      ...
      const totalRemaining = Math.max(0, totalPaid + totalScholarship - totalFee);
...
app.post("/change-password", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.execute("SELECT password FROM users WHERE id = ?", [req.session.user.id]);
    if (rows.length === 0) {
...
app.get("/admin/api/rooms", checkAdmin, async (req, res) => {
  try {
    ...
    let [rooms] = await db.query("SELECT * FROM rooms ORDER BY hostel_name, floor, room_no");
    ...
    if (roll_no) {
      ...
      if (hostel_name && hostel_name !== "all") filteredRooms = filteredRooms.filter(r => r.hostel_name === hostel_name);
      if (floor && floor !== "all") filteredRooms = filteredRooms.filter(r => r.floor === floor);
    }
...
app.post("/admin/api/rooms/update/:id", checkAdmin, async (req, res) => {
  ...
  try {
    await db.query("UPDATE rooms SET capacity=? WHERE id=?", [capacity, roomId]);
...
app.get('/admin/applications/:id', checkAdmin, async (req, res) => {
  try {
    ...
    const [rows] = await db.query('SELECT * FROM hostel_applications WHERE application_id=?', [id]);
...
app.post("/admin/api/billing/add", checkAdmin, async (req,res)=>{
  const {
    roll_no, student_name, room_no, hostel_name, course, branch, year,
    academic_year, month, hostel_fee=0, monthly_fee=0, paid_amount=0, scholarship=0,
    food_preference="Veg"
  }=req.body;

  if(!roll_no || !month || !academic_year)
    return res.json({success:false,message:"Roll, month, and academic year required"});

  try{
    const normalizedMonth = normalizeMonth(month);
    if(!normalizedMonth) return res.json({success:false,message:"Invalid month"});
    ...
    const [result]=await db.query(`
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `, [roll_no, student_name, room_no, hostel_name, course, branch, year, academic_year, normalizedMonth, hostel_fee, monthly_fee, paid_amount, scholarship, food_preference]);
...
app.post("/admin/api/billing/saveYear", checkAdmin, async (req, res) => {
  ...
  try {
    ...
    await conn.query("DELETE FROM student_billing WHERE roll_no=? AND academic_year=?", [roll_no, academic_year]);
...
app.get("/api/students", async (req, res) => {
  const [rows] = await db.query("SELECT * FROM hostel_applications ORDER BY roll_no");
  res.json(rows);
});

// ======================= GET SINGLE STUDENT =======================
app.get("/api/students/:roll_no", async (req, res) => {
  const [rows] = await db.query("SELECT * FROM hostel_applications WHERE roll_no=?", [req.params.roll_no]);
  if (rows.length) res.json(rows[0]);
  else res.status(404).json({ message: "Student not found" });
});

// ======================= ADD STUDENT =======================
app.post("/api/students", upload.fields([{ name: "student_photo" }, { name: "certificate" }]), async (req, res) => {
  try {
    const body = req.body;

    const [exists] = await db.query("SELECT * FROM hostel_applications WHERE roll_no=?", [body.roll_no]);
    if (exists.length) return res.status(400).json({ message: "Roll number exists" });
    ...
    if (req.files["student_photo"]) filteredBody.student_photo = req.files["student_photo"][0].filename;
    if (req.files["certificate"]) filteredBody.certificate = req.files["certificate"][0].filename;
    ...
    await db.query(`INSERT INTO hostel_applications (${fields.join(",")}) VALUES (${placeholders})`, values);
    res.sendStatus(200);
  } catch (err) {
    console.error(err);
...
// ======================= EDIT STUDENT =======================
app.put("/api/students/:roll_no", upload.fields([{ name: "student_photo" }, { name: "certificate" }]), async (req, res) => {
  try {
    const roll_no = req.params.roll_no;
    const [existing] = await db.query("SELECT * FROM hostel_applications WHERE roll_no=?", [roll_no]);
    ...
    Object.keys(old).forEach(k => {
      filteredBody[k] = (body[k] !== undefined && body[k] !== "") ? body[k] : old[k];
    });
    ...
    await db.query(`UPDATE hostel_applications SET ${setStr} WHERE roll_no=?`, [...values, roll_no]);
...
app.delete("/api/students/:roll_no", async (req, res) => {
  try {
    await db.query("DELETE FROM hostel_applications WHERE roll_no=?", [req.params.roll_no]);
    res.sendStatus(200);
  } catch (err) {
    console.error(err);
...
app.delete("/api/complaints/:id", async (req, res) => {
  try {
    const [result] = await db.query("DELETE FROM complaints WHERE complaint_id=?", [req.params.id]);
    if (result.affectedRows === 0) return res.status(404).json({ message: "Complaint not found" });
